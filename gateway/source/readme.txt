...
/************************************************ 0925**********************************************************/
重新设计键值的逻辑，在键值解析和键值执行之间增加一个键值逻辑处理。键值逻辑处理使用过滤器架构，依次经过，插卡判断，
互斥判断，延时判断映射判断。
1、增加一个键值处理文件，剥离键值功能
2、增加键值的逻辑  分离解析键值  和执行键值
3、插卡 拔卡 判断  延时发送,增加映射 插卡拔卡判断  
4、映射控制：插卡拔卡判断正确 ，普通控制正确， 延时恢复正确（人体感应  门铃）， 延时触发正确（类似拔卡操作）
5、增加软件版本号上报

/************************************************ 0926**********************************************************/
修改GPIO引脚  PB3配置  5个输出口都正常使用
增加上电后发场景2
修改按键判断时间 20改 5
修改IO输入自恢复类型控制
增加插卡取反操作
